...
public interface UsersApi {
    ...
    @GetMapping(value =  "/users/{idUsers}", produces = MediaType.APPLICATION_JSON_VALUE)
    UsersDto findById(@PathVariable("idUsers") Long id);

    @GetMapping(value =  "/users/all", produces = MediaType.APPLICATION_JSON_VALUE)
    List<UsersDto> findAll();

    @GetMapping(value =  "/users/delete/{idUsers}", produces = MediaType.APPLICATION_JSON_VALUE)
